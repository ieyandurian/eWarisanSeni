						...
						<div id="content">
							<div class="breadcrumb">
								<a href="ViewCustomer/home.jsp">Home</a> &raquo; <a
									...
									href="ViewCustomer/address.jsp">Address Book</a>
							</div>
							<h1>Address Book</h1>
							<sql:setDataSource var="snapshot"
								driver="oracle.jdbc.driver.OracleDriver"
							...
							<h4>Sorry, insert new address has failed.</h4>
							<c:forEach items="${result.rows}" var="row">
								<div class="content">
									<table style="width: 100%;">
										<tr>
											<td><b><%=currentUser.getCust_firstname() + " "
						+ currentUser.getCust_lastname()%>,</b></td>
										</tr>
										<tr>
											<td><c:out value="${row.add_address}" />,<br> <c:out
													value="${row.add_postcode}" />, <c:out
													value="${row.add_city}" />,<br> <c:out
													value="${row.add_state}" />,<br> <c:out
													value="${row.add_country}" /></td>
										</tr>
										<tr>
											<td style="text-align: right;"><a
												href="ViewCustomer/editaddress.jsp?add_id=<c:out value="${row.add_id}"/>"
												class="button">Edit</a>&nbsp; <a class="button"
												href="javascript:confirmGo('Sure to delete this address?','deleteAddress?add_id=<c:out value="${row.add_id}"/>')">Delete</a>
										</tr>
									</table>
								</div>
							</c:forEach>
							<div class="buttons">
								...
								<div class="right">
									<a href="ViewCustomer/insertaddress.jsp" class="button">New
										Address</a>
								</div>
							</div>
						</div>
